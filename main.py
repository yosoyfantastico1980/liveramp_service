...
from models import ErrorResponse, DestinationsResponse, SegmentsResponse
...
class TokenManager:
    ...
    def _fetch_new_token(self) -> Dict[str, Any]:
        """
        POST https://serviceaccounts.liveramp.com/authn/v1/oauth2/token
        Content-Type: application/x-www-form-urlencoded
        """
        base = "https://serviceaccounts.liveramp.com"
        path = "/authn/v1/oauth2/token"
        url = base + path

        username = os.getenv("LR_USERNAME")
        password = os.getenv("LR_PASSWORD")
        client_id = os.getenv("LR_CLIENT_ID", "liveramp-api")

        if not username or not password:
            raise HTTPException(
                status_code=500,
                detail="Server is missing LR_USERNAME and/or LR_PASSWORD env vars",
            )
    ...
    def get_access_token(self) -> str:
        now = time.time()
        # refresh ~60s early to avoid edge-of-expiration failures
...
def resolve_org_id(x_lr_org_id: Optional[str]) -> str:
    ...
    Org resolution:
      ...
      2) Otherwise fall back to LR_ORG_ID env var.
    """
    env_org = os.getenv("LR_ORG_ID")
    ...
    if not env_org:
        raise HTTPException(status_code=500, detail="Server is missing LR_ORG_ID env var")
    return env_org
...
@v1.get(
    ...
    responses={502: {"model": ErrorResponse}, 401: {"model": ErrorResponse}, 403: {"model": ErrorResponse}},
)
def list_destinations(
    limit: int = Query(1, ge=1, le=200),
    after: Optional[str] = Query(default=None, description="Pagination cursor from prior response _pagination.after"),
    x_lr_org_id: Optional[str] = Header(default=None, alias="X-LR-Org-Id"),
):
    org_id = resolve_org_id(x_lr_org_id)
    ...
    return lr_request("GET", "/activation/v2/destinations", org_id, query_params=qp)
...
def list_first_party_segments(
    ...
    if after:
        qp.append(("after", after))
    return lr_request("GET", "/activation/v2/segments", org_id, query_params=qp)


@v1.get("/liveramp/marketplace/segments", dependencies=[Depends(require_pp_api_key_security)])
def list_marketplace_segments(
    ...
    return lr_request("GET", "/data-marketplace/buyer-api/v3/segments", org_id, query_params=qp)


@v1.get("/liveramp/marketplace/segments/detail", dependencies=[Depends(require_pp_api_key_security)])
...
@v1.post("/liveramp/requested-segments", dependencies=[Depends(require_pp_api_key_security)])
def request_segments_for_activation(
    payload: List[Dict[str, Any]] = Body(...),
    x_lr_org_id: Optional[str] = Header(default=None, alias="X-LR-Org-Id"),
):
    org_id = resolve_org_id(x_lr_org_id)
    return lr_request("POST", "/data-marketplace/buyer-api/v3/requested-segments", org_id, json_body=payload)


@v1.get("/liveramp/segment-statuses", dependencies=[Depends(require_pp_api_key_security)])
def segment_statuses(
    ...
    qp: List[Tuple[str, Union[str, int]]] = [("segmentType", segmentType), ("limit", limit)]
    for seg_id in segmentIDs:
        qp.append(("segmentIDs[]", seg_id))

    return lr_request("GET", "/activation/v2/segment-statuses", org_id, query_params=qp)


@v1.get("/liveramp/deliveries", dependencies=[Depends(require_pp_api_key_security)])
def deliveries(
    integrationConnectionID: str = Query(...),
    x_lr_org_id: Optional[str] = Header(default=None, alias="X-LR-Org-Id"),
):
    org_id = resolve_org_id(x_lr_org_id)
    qp: List[Tuple[str, Union[str, int]]] = [("integrationConnectionID", integrationConnectionID)]
    return lr_request("GET", "/activation/v2/deliveries", org_id, query_params=qp)
